...
def run_test(filename, description, requests, replicas, mayfail, tolerance, latency, log=None):
    ...
    if args.config_dir:
        sim = Simulation(os.path.join(args.config_dir, filename))
    else:
    ...
    if stats.died:
        print
        '\t\tTesting error: >0 replicas died unexpectedly'
        pf = 'FAIL'
    ...
    if stats.total_msgs > requests * replicas * 2 * tolerance:
        print
        '\t\tTesting error: Too many total messages'
        pf = 'FAIL'
    if stats.mean_latency > latency:
        print
        '\t\tTesting error: Latency of requests is too high'
        pf = 'FAIL'
...
print
'Passed', sum([1 for x in trials if x]), 'out of', len(trials), 'tests'
