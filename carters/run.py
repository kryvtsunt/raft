...
class Client:
    ...
    def deliver(self, raw_msg, msg):
        ...
        if 'MID' not in msg:
            print
            "*** Simulator Error - Message missing mid field: %s" % (raw_msg)
            self.sim.stats.incorrect += 1
            return None
        if msg['type'] not in ['ok', 'fail', 'redirect']:
            print
            "*** Simulator Error - Unknown message type sent to client: %s" % (raw_msg)
            self.sim.stats.incorrect += 1
            return None
        ...
        try:
            ...
            print
            "*** Simulator Error - client received an unexpected MID: %s" % (raw_msg)
            self.sim.stats.incorrect += 1
            return None
        ...
        if req.get:
            if 'value' not in msg:
                print
                "*** Simulator Error - get() response missing the value of the key: %s" % (raw_msg)
                self.sim.stats.incorrect += 1

            if self.items[req.key] != msg['value']:
                print
                "*** Simulator Error - client received an incorrect value for a key: %s" % (raw_msg)
                self.sim.stats.incorrect += 1
        else:
...
class Simulation:
    ...
    def __route_msgs__(self, sock):
        try:
            ...
            print
            "*** Simulator Error - A replica quit unexpectedly"
        ...
        if len(raw_msg) == 0:
            print
            "*** Simulator Error - Replica shut down a socket unexpectedly"
            self.__close_replica__(sock)
            return
        ...
        try:
            ...
            print
            "*** Simulator Error - Unable to decode JSON message: %s" % (raw_msg)
            self.stats.incorrect += 1
            return

        if type(msg) is not dict:
            print
            "*** Simulator Error - Message is not a dictionary: %s" % (raw_msg)
            self.stats.incorrect += 1
            return
        if 'src' not in msg or 'dst' not in msg or 'leader' not in msg or 'type' not in msg:
            print
            "*** Simulator Error - Message is missing a required field: %s" % (raw_msg)
            self.stats.incorrect += 1
            return
        if not self.__validate_addr__(msg['leader']):
            print
            "*** Simulator Error - Incorrect leader format: %s" % (raw_msg)
            self.stats.incorrect += 1
            return
        if not self.__validate_addr__(msg['dst']):
            print
            "*** Simulator Error - Incorrect destination format: %s" % (raw_msg)
            self.stats.incorrect += 1
            return
        if not self.__validate_addr__(msg['src']):
            print
            "*** Simulator Error - Incorrect source format: %s" % (raw_msg)
            self.stats.incorrect += 1
            return
        ...
        # we have no idea who the destination is
            ...
            print
            "*** Simulator Error - Unknown destination: %s" % (raw_msg)
            self.stats.incorrect += 1
